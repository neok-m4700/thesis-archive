...
At(1,1,l) = 0;
Ab(1,1,l) = k_c*(dx*dy/dz)/dz;

Apn(1,1,l) = 0; %rho*c_sp*dx*dy*dz/dt; Using SS
bp(1,1,l) = Sc(1,1,l)*dx*dy*dz+Apn(1,1,l)*Tn(1,1,l);
Ap(1,1,l) = An(1,1,l)+As(1,1,l)+Aw(1,1,l)+Ae(1,1,l)+...
    At(1,1,l)+Ab(1,1,l)+Apn(1,1,l)-Sp(1,1,l)*dx*dy*dz;

% West North Top
An(1,n,l) = 0;
As(1,n,l) = k_c*(dx*dz/dy)/dy;
Ae(1,n,l) = k_c*(dy*dz/dx)/dx;
Aw(1,n,l) = 0;
At(1,n,l) = 0;
Ab(1,n,l) = k_c*(dx*dy/dz)/dz;

Apn(1,n,l) = 0; %rho*c_sp*dx*dy*dz/dt; Using SS
bp(1,n,l) = Sc(1,n,l)*dx*dy*dz+Apn(1,n,l)*Tn(1,n,l);
Ap(1,n,l) = An(1,n,l)+As(1,n,l)+Aw(1,n,l)+Ae(1,n,l)+...
    At(1,n,l)+Ab(1,n,l)+Apn(1,n,l)-Sp(1,n,l)*dx*dy*dz;

% North East Top
An(m,n,l) = 0;
As(m,n,l) = k_c*(dx*dz/dy)/dy;
Ae(m,n,l) = 0;
Aw(m,n,l) = k_c*(dy*dz/dx)/dx;
At(m,n,l) = 0;
Ab(m,n,l) = k_c*(dx*dy/dz)/dz;

Apn(m,n,l) = 0; %rho*c_sp*dx*dy*dz/dt; Using SS
bp(m,n,l) = Sc(m,n,l)*dx*dy*dz+Apn(m,n,l)*Tn(m,n,l);
Ap(m,n,l) = An(m,n,l)+As(m,n,l)+Aw(m,n,l)+Ae(m,n,l)+...
    At(m,n,l)+Ab(m,n,l)+Apn(m,n,l)-Sp(m,n,l)*dx*dy*dz;

% East South Top
An(m,1,l) = k_c*(dx*dz/dy)/dy;
As(m,1,l) = 0;
Ae(m,1,l) = 0;
Aw(m,1,l) = k_c*(dy*dz/dx)/dx;
At(m,1,l) = 0;
Ab(m,1,l) = k_c*(dx*dy/dz)/dz;

Apn(m,1,l) = 0; %rho*c_sp*dx*dy*dz/dt; Using SS
bp(m,1,l) = Sc(m,1,l)*dx*dy*dz+Apn(m,1,l)*Tn(m,1,l);
Ap(m,1,l) = An(m,1,l)+As(m,1,l)+Aw(m,1,l)+Ae(m,1,l)+...
    At(m,1,l)+Ab(m,1,l)+Apn(m,1,l)-Sp(m,1,l)*dx*dy*dz;
